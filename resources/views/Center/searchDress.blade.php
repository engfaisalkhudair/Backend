...
@section('content')
    ...
    <div class="card mb-5 mb-xl-10" style="margin-right: 20px;margin-left: 20px;">
        <div class="row g-4">
            ...
            <!-- Modal -->
            @endif
               <div class="card-body row border-top p-9">
                    <div class="col-sm-auto">
                            <form action="{{ route('dress.searchAtName') }}" method="post" style="margin-bottom: 10px;">
                                @csrf
                                <input type="search" style="
                                    ...
                                    transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out,
                                "  name="search_name" class="form-control" id="searchcategory" placeholder="أبحث ب  اسم الفستان...">

                            </form>
                    </div>
                    <div class="col-sm-auto">
                        <form action="{{ route('dress.searchAtNameClient') }}" method="post" style="margin-bottom: 10px;">
                            @csrf
                            <input type="search" style="
                                padding: .5rem .9rem;
                                font-size: 12px;
                                ...
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out;
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out,
                            "  name="search_name_client" class="form-control" id="searchcategory" placeholder="أبحث ب  اسم الزبون ...">

                        </form>
                </div>
                    <div class="col-sm-auto">

                        <form action="{{ route('dress.searchAtCode') }}" method="post" style="margin-bottom: 10px;">
                            @csrf
                            <input type="search" style="
                                padding: .5rem .9rem;
                                font-size: 12px;
                                ...
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out;
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out,
                            "  name="search_code" class="form-control" id="searchcategory" placeholder="أبحث ب كود الفستان ...">
                            <i class="ri-search-line search-icon"></i>
                        </form>
                    </div>
                    <div class="col-sm-auto">

                        <form action="{{ route('dress.searchAtDate') }}" method="post" style="margin-bottom: 10px;">
                            @csrf
                          <div class="row">
                          <input type="date" style="
                                ...
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out;
                                transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out,
                            "  name="search_date" class="form-control" id="searchcategory" placeholder="أبحث ب تاريخ الحجز ...">
                            <button class=" btn btn-primary" style="margin-top:10px; font-size:12px;">أنقر هنا بعد اختيار التاريخ </button>
                            {{-- <i class="ri-search-line search-icon"></i> --}}
                          </div>
                        </form>
                    </div>
            ...
            <div class="card-body pt-3">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                        ...
                        <!--begin::Table body-->
                        <tbody>
                            @foreach ($filtersDress as $itemdress)
                                <tr>
                                    <td>
                                ...
                                <div class="modal fade" id="exampleModal{{  $itemdress->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        <form action="{{ route("dress.destroy")}}" method="post">
                                            ...
                                            <button type="submit" class="btn btn-primary">انهاء الحجز</button>
                                            </div>
                                        </form>
                                    </div>
                                    </div>
    ...
    <div class="g-5 gx-xxl-8" style="margin-bottom:80px">
               ...
               <div class="card">
                ...
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column" style="padding-bottom: 15px">
                        <span class="card-label fw-bold fs-3 mb-1">  نتائج البحث عن الحجوزات  </span>
                    </h3>
                ...
                <div class="card-body pt-3">
                    <!--begin::Table container-->
                    <div class="table-responsive">
                        يمكنك البحث عن الحجز باحد الطرق الموضحة في الاعلى
                        <!--end::Table-->
                    </div>
                    <!--end::Table container-->
                </div>
